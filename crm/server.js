...
app.get("/bootstrap.js", requireAuth, async (request, response, next) => {
  try {
    ...
    response.type("application/javascript").send(`window.__NAZOFT_AUTHENTICATED__=true;window.__NAZOFT_REMOTE_STATE__=${JSON.stringify(workspace.state).replaceAll("<", "\\u003c")};`);
  } catch (error) { next(error); }
});
app.get("/bootstrap.js", (_request, response) => {
  response.type("application/javascript").send("window.__NAZOFT_AUTHENTICATED__=false;window.__NAZOFT_REMOTE_STATE__=null;");
});
